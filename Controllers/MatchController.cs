...
namespace PubgTournament.Controllers
{
    ...
    {
        private readonly IStore<Match> _store;
        [JsonConstructorAttribute]
        public MatchController(IStore<Match> store)
        {
            _store = store;
        }
        // [Authorize]
        [HttpPost]
        [Route("add")]
        public async Task<IActionResult> AddAsync(Match model)
        {
            model.Id =  Guid.NewGuid().ToString();
            await _store.InsertOneAsync(model);
            return Ok(CreateSuccessResponse(model));
        }

        [HttpPost]
        [Route("update")]
        public async Task<IActionResult> UpdateAsync(Match model)
        {
